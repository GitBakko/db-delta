...
- **925 test verdi** nei sette progetti che girano senza Docker (Core 544,
  Headless 208, Persistence.Unit 81, Golden 68, Property 12, Architecture 6,
  Shared 6) — ricontati il 2026-08-31 dopo la chiusura del tipo
  memory-optimized, non incrementati a mente. **Due** dei tre DB-backed vanno **rossi**, non skipped, con Docker
...
- **14 voci aperte** — P1 3 · P2 1 · P3 2 · P4 1 · P5 7 — più **17** in
  «Deciso — NON riaprire». **Le 4 critiche restano chiuse.** Il «19» che questa
  riga portava fino al 2026-08-31 era scollato: le righe di quella tabella sono
  **17**, contate una per una. Per origine le 14 aperte sono: **cinque**
  dall'audit di parità Redgate (P1 DROP TYPE nudo, P1 rebuild contro un modulo
  schemabound, P2 CHECK in linea, P3 gate d'errore di batch, P4 politica delle
  DROP) — **nessuna è un fallimento di parità**, sono forme che la fixture non
  raggiunge; **due** dalla review adversariale della chiusura del tipo tabella
  (il nome di un tipo alias senza schema, la direzione delle colonne di chiave
  persa anche sulle tabelle), entrambe **pre-esistenti e rese visibili dalla
  correzione**; **sette** del proprietario, riverificate sul codice il
  2026-08-18. Delle sei che l'audit di parità aveva aperto ne restano quattro
  più quella d'igiene: le chiavi del tipo tabella perse in silenzio e il tipo
  memory-optimized riscritto su disco sono chiuse, entrambe in P1. Il
...
- **CI verde su `3c48735`** (run `32140004994`), entrambi i job — **quel run è
  anteriore ai commit locali e non li ha visti**. I DB-backed aggiungono **84**
  test ai locali della riga sopra: LiveDb 54, Cli acceptance 23, Persistence
  integration 7. Misurati il 2026-08-31 su Windows con Docker acceso, dove i 7
  passano tutti; senza Docker 3 dei 7 si skippano da sé.
...
Due voci chiuse il 2026-08-31, ognuna dal commit che porta la sua riga:
...
|---|---|---|---|
| **Il nome di un tipo ALIAS esce senza schema, e il deploy si ferma.** Tutti i reader leggono `TYPE_NAME(user_type_id)`, che torna il nome nudo: una colonna di tipo `app.MioTipo` viene emessa come `[Col] [MioTipo]` e su un target dove quel tipo non sta nello schema di default il `CREATE` muore. **Pre-esistente e condiviso**, non introdotto dalle chiavi del tipo tabella: `TableReader.cs:36` fa esattamente lo stesso di `TableTypeUdtReader.cs:47`, quindi vale per le colonne di ogni tabella. Serve `SCHEMA_NAME(ty.schema_id)` accanto al nome, e i due emittenti che scrivono colonne devono qualificare | 2026-08-31 | S | `src/DbDelta.Providers.LiveDb/Readers/TableReader.cs:36` e `TableTypeUdtReader.cs:40`; `src/DbDelta.Core/ScriptGen/SqlTypeFormatter.cs` |
