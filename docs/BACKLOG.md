...
- **890 test verdi** nei sette progetti che girano senza Docker (Core 514,
...
- **CI verde su `3c48735`** (run `32140004994`), entrambi i job. I DB-backed
  aggiungono **67** test ai locali della riga sopra: LiveDb 38, Cli acceptance
...
| **L'invariante di convergenza copre 4 kind su 14** (View, Procedure, **Function e Trigger dal 2026-08-20**). I quattro sono i kind *modulari*: il modello porta il corpo come testo, quindi «emetti, rileggi, deve essere Identical» si scrive in un test di unità. **Gli altri dieci non si possono coprire qui**: rileggere una tabella o una sequenza dal proprio DDL vorrebbe dire riscrivere un parser di T-SQL, e la strada vera è il round-trip dal vivo — è la voce qui sopra, non una seconda copia di questa | 2026-08-01 | M | `Core.UnitTests/Diff/DeployedModuleConvergesTests.cs` — 33 test. **Sonda di mutazione fatta:** tolto il taglio dei terminatori ripetuti in `BodyNormalizer` ne cadono 12, Function e Trigger compresi. Nota utile: rompendo *solo* l'emittente non cade nessuna convergenza — il normalizzatore la copre — quindi le due metà hanno anche i loro test diretti, ed è di proposito |
