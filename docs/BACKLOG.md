...
- **908 test verdi** nei sette progetti che girano senza Docker (Core 530,
  Headless 206, Persistence.Unit 80, Golden 68, Property 12, Architecture 6,
...
| Voce | Reg. | Sforzo | Evidenza verificata |
|---|---|---|---|
| **Restano 5 view irraggiungibili.** Il connection manager è stato legato a un pulsante il 2026-08-20 e non è più fra queste; `ConnectionPickerView` resta codice morto ma non è più una superficie che pubblica una password. Le altre — `ResultsTreeView` fra loro — non hanno un punto d'ingresso e nessuno ne ha chiesto uno: o si legano, o si cancellano | 2026-08-14 | M | `Views/ConnectionPickerView.axaml`, `Views/ResultsTreeView.axaml` |
| **`ProjectOptions` e le mappature owner/table non raggiungono alcun motore.** Sono cinque booleani e due liste che l'app fa girare fra il dialogo di progetto e il file `.dbd`, e che nessuno legge al momento del confronto. **Non le tocco insieme ai flag**: sono dati che un utente può avere già salvato, e cancellarle cambia il formato del `.dbd`. Il dialogo per ora non le espone, quindi oggi non sono una bugia verso l'utente — lo diventano il giorno che qualcuno ci mette una casella | 2026-08-14 | S | `Core/Abstractions/ProjectOptions.cs`, `Core/Abstractions/TableMappingEntry.cs`; girano in `ViewModels/ProjectSetupViewModel.cs:246-268`, mai in `ComparisonEngine` |
| **`LiveDbObjectBodyResolver`, 764 righe**, apre due connessioni per clic e fino a 16 query. **Le due ragioni che questa voce dava non esistono più:** i pannelli vuoti per `TableType` e `Schema` sono chiusi (2026-08-20), e la virtualizzazione dei pannelli diff — che la voce dichiarava bloccata qui — è stata chiusa senza toccare questo file, perché il costo era il layout e non il caricamento. Resta la dimensione e il numero di round-trip per clic, cioè **debito di forma, non un difetto che qualcuno subisce**: a ritmo di clic umano sedici query non si sentono. Decidere se vale una riscrittura o se la voce va archiviata | 2026-08-14 | L | `Providers.LiveDb/ObjectBody/LiveDbObjectBodyResolver.cs:33`, `:35-49` (lo switch), `:277-321` (il corpo tabella) |
