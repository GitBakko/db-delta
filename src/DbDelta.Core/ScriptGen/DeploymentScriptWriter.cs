...
public sealed class DeploymentScriptWriter(StringBuilder sb, bool useTransaction)
{
    ...
    /// <summary>
    /// The other half of the same declaration: this script was generated to run
    /// with NO transaction at all, on purpose. Without it a script built with
    /// <c>ComparisonOptions.NoTransactions</c> was indistinguishable from a
    /// hand-written one, so <c>dbdelta apply</c> gave it a client transaction and
    /// undid the very option that had generated it — the operator had to pass
    /// <c>--no-transaction</c> as well to get what they had already asked for.
    /// Emitted regardless of <c>includeHeader</c>, for the same reason as its
    /// twin: a saved script still has to be classifiable.
    /// </summary>
    public const string NoTransactionMarker = "-- dbdelta:transaction=none";

    public void WritePreamble(bool includeHeader)
    {
        sb.AppendLine(useTransaction ? SelfManagedTransactionMarker : NoTransactionMarker);
