...
public sealed class ScriptGenerator
{
        ...
        string? targetDefaultCollation = null)
    {
        ...
        // 0.9 Drop inbound FKs to tables that are about to be rebuilt
        //     (M13-PARITY.6 #33). Section 7 below skips these names so the
        ...
        //    FKs whose names are already covered by the rebuild orchestrator
        //    (section 0.9 drop + section 7.9 add) are skipped here to avoid
        //    double drops / adds (M13-PARITY.6 #33).
        foreach (DifferencePair pair in pairs.Where(p => p.Identity.Kind == "Table"))
        {
            switch (pair.Status)
            {
        ...
        // 7.9 Re-add inbound FKs to rebuilt tables (M13-PARITY.6 #33). Source
        //     side is authoritative — these are the FKs the final shape
        //     requires. Section 7 above skipped them to keep this single
        //     re-add path clean.
